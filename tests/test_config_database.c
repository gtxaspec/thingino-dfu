...
int main(void) {
    ...
    for (size_t i = 0; i < sizeof(test_configs) / sizeof(test_configs[0]); i++) {
        ...
        ddr_phy_params_t phy_params = {
            ...
            .row_bits = ddr_cfg->row,
            .col_bits = ddr_cfg->col,
            .cl = ddr_cfg->cl,
            .bl = ddr_cfg->bl,
        ...
        // Generate binary
        uint8_t *ddr_binary = (uint8_t*)malloc(DDR_BINARY_SIZE_MAX);
        if (!ddr_binary) {
    ...
    if (failures > 0)
        printf("\n[FAIL] Configuration database test: %d failure(s)\n", failures);
    else
