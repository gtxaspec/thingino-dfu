...
static void test_null_path(void) {
    ...
    tdfu_error_t result = firmware_file_check_readable(NULL);
    TEST(result == TDFU_ERROR_INVALID_PARAMETER,
...
static void test_nonexistent_file(void) {
    ...
    tdfu_error_t result = firmware_file_check_readable("/nonexistent/path/firmware.bin");
    TEST(result == TDFU_ERROR_FILE_IO,
...
static void test_valid_file(void) {
    ...
    const char *tmp_path = "/tmp/tdfu_test_valid_fw.bin";
    ...
    if (f) {
        ...
        tdfu_error_t result = firmware_file_check_readable(tmp_path);
        TEST(result == TDFU_SUCCESS,
...
static void test_load_file_fail(void) {
    ...
    uint8_t *data = NULL;
    size_t size = 0;
    tdfu_error_t result = load_file("/nonexistent/path/spl.bin", &data, &size);
    TEST(result == TDFU_ERROR_FILE_IO,
