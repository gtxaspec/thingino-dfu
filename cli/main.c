...
int main(int argc, char *argv[]) {
    ...
    // Remote mode: dispatch to the dfu-remote daemon
    if (options.remote_host) {
        ...
        int rc = 0;
        /* Variant detection probes a bootrom; it's needed for DFU bootstrap but
         * NOT for a DFU read/write (those target an already-running U-Boot DFU
         * gadget, which has no SoC variant). */
        ...
        bool dfu_custom_blobs = options.spl_file && options.spl_file[0] &&
                                options.uboot_file && options.uboot_file[0];
        bool need_variant = options.bootstrap && !dfu_custom_blobs;
        if (need_variant && !cpu) {
    ...
    /* -l: list every connected Ingenic device (bootrom AND DFU). Probing only
     * for a DFU gadget can't see a bootrom device - and the probe's recovery
     * path would issue a needless USB reset on it. So print the inventory, then
     * show DFU alt-settings only when the targeted device is actually a gadget. */
    if (options.list_devices) {
